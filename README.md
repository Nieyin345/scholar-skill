...
| 流程四 论文写作 | 模板即论文（thesis/template/）：结构以模板为准、防 AI 味；两阶段：阶段A 中文 MD 按模板结构定稿（写→润色→内容 Review，未定稿不翻译）→ 阶段B 翻译英文 LaTeX + 翻译质量 Review 循环 → 编译；review 发现问题非写作类则中断返回前流程；历史版本与 review 反馈存档 |
...
│       ├── 00_研究方向.md
│       ├── thesis/           # 论文 LaTeX 模板（流程四：用户模板或默认模板副本）
│       ├── <论文短名-年>/    # 每篇论文一个文件夹：pdf + md
│       ├── 03_文献综述.md
│       ├── 解决方案/         # 01 场景分析 / 02 实现逻辑 / 03 代码架构
│       ├── implementation/   # 流程三代码实现 + 实验图 figures/
│       ├── thesis/           # 流程四论文工作目录（template/ 原始模板参考只读 + 工作副本 + 写作产物 + versions/）
