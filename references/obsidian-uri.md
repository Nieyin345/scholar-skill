...
## 6. 何时触发（模型判断规则，先判断再调用）

**默认不调 URI**：例行生成 / 更新 md 文档一律直接写文件（零依赖、可靠）；只有判断「这次调用对用户有实际价值」才触发。调用前先自问：不调会怎样？如果只是「打开个东西看看」，就直接写文件并告知路径，让用户自己点。

**该触发（正例）**：

- 生成 / 更新笔记后，用户正在 Obsidian 里工作 → 打开该笔记定位到对应章节（`filepath` + `heading`/`line`），方便用户立刻查看；
- 用户**明确要求**用某个插件 / 在 Obsidian 里查看 / 走某个插件工作流；
- 批量改了 frontmatter 属性，且用户在用 Dataview / Bases 视图 → 刷新视图（`dataview:dataview-pane-open` 等），让表格数据即时更新；
- 用户在用 obsidian-git 同步，且刚完成一批笔记变更 → **先征询**，同意后 `obsidian-git:push`；
- 用户配置了 Templater 模板并明确要套用 → 新建笔记时 `templater-obsidian:insert-templater`。

**不该触发（反例）**：

- 例行写 md 文件（默认路径，不调）；
- 无明显受益的调用（「能调所以调」）；
- 会造成打扰的动作：乱弹标签页、未经确认就 git push / 覆盖属性 / 插入模板（URI 会在用户正打开的 Obsidian 里即时生效，副作用类操作先告知或征询）；
- 插件未装 → 不调，改走「提醒安装」流程（见 §1）。

**副作用命令原则**：git push / 插入模板 / 写属性 / 运行宏等一次性副作用，执行前先向用户说明将做什么（或直接征询确认）；打开/定位类（只改变视图）可直接执行。
