...
| 规范文件 | 对应流程 | 写作任务 / 产出文件 |
|----------|----------|----------------------|
| `通用写作规范.md` | 全部流程 | 跨任务底线：反 AI 腔 / 过度声称护栏 / 术语保护 / 完整性 / 先读后写 / 数学公式 LaTeX+Obsidian（v1.6，所有写作任务同时执行） |
| `研究方向确认规范.md` | 流程0 | `论文/<主题>/00_研究方向.md` + `00_研究问题卡.md`（v2.0：SMART 问题/证据记录/方法路线/Proposal Readiness Gate） |
| `综述写作规范.md` | 流程一 | `03_文献综述.md`（v3.4：先读后写铁律——逐篇精读全文 + Evidence Record 已读证据，未精读不得引用） |
| `解决方案-场景分析与问题剖析.md` | 流程二 | `解决方案/01_场景分析与问题剖析.md`（v2.2：先读后写——已有研究对比/空白判断必须来自已读文献） |
| `解决方案-实现逻辑与方案设计.md` | 流程二 | `解决方案/02_实现逻辑与方案设计-方案X.md`（v3.2 方法论：问题类型决策树、资源适配、4 阶段渐进实验、一个方案一篇、逐篇生成 + 先读后写） |
| `解决方案-代码架构设计.md` | 流程二 | `解决方案/03_代码架构设计.md`（v3.3 架构层面 + 学术场景适配 + 实现顺序对齐 4 阶段实验 + 评审门禁 + 先读后写） |
| `代码实现规范.md` | 流程三 | 按 03 架构生成的代码文件与测试（v1.2：TDD + 编码规范 + 系统化调试 + 验证证据门禁 + 梯子模型最小化决策 + 简化审查） |
| `论文写作规范.md` | 流程四 | `论文/<主题>/thesis/` 内 `main.tex` + 编译 `main.pdf`（v2.4：两阶段写作 + 内容 Review/翻译 Review + 模板符合度对照 + 主张-证据纪律 + 引用禁编造；历史版本与 review 反馈存档于 `thesis/versions/`） |
